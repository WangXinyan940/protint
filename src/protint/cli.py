...
from .workflow.predict import predict_single, predict_pkl, predict_directory, predict_pairs_directory, load_model
...
def parse_pdb_dataset(args: argparse.Namespace):
    """Generate embeddings for all proteins in a dataset and create pairs.csv.
    ...
    Output:
    - Individual pkl files for each protein
    - pairs.csv: Copy of targets.csv with pkl filename references
    """
    import pandas as pd
    import shutil
    ...
    pairs_output = output_dir / "pairs.csv"
    pairs_df[['antibody_pkl', 'antigen_pkl', 'classification_label']].to_csv(
        pairs_output, index=False
    )
    print(f"\nSaved pairing info to: {pairs_output}")
...
def train_model(args: argparse.Namespace):
    ...
    - pairs.csv with columns: antibody_pkl, antigen_pkl, classification_label
    """
    print("=" * 60)
...
def predict(args: argparse.Namespace):
    ...
    Supports two modes:
    1. Directory with pairs.csv: Predicts all pairs defined in pairs.csv
    ...
    pairs_csv = input_path / "pairs.csv" if input_path.is_dir() else None

    if pairs_csv and pairs_csv.exists():
        # Dataset mode: use pairs.csv
        print(f"Predicting on dataset directory: {args.input}")
...
def entry():
    ...
    gen_embed_parser.add_argument("-o", "--output", type=Path, required=True, help="Path to the output folder for saving embeddings and pairs.csv.")
    ...
    train_parser.add_argument("--train-dir", type=Path, required=True, help="Directory containing training pkl files and pairs.csv")
    train_parser.add_argument("--val-dir", type=Path, help="Directory containing validation pkl files and pairs.csv")
    ...
    predict_parser.add_argument("-o", "--output", type=Path, help="Output file for results")
